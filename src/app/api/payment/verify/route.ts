...
export async function POST(req: Request) {
  ...
  if (!valid) {
    // Only a payment that is not already captured can be marked failed. Order
    // ids are not secret, so without this guard an unauthenticated request
    // could flip somebody's successful payment to failed.
    await db
      .from('payments')
